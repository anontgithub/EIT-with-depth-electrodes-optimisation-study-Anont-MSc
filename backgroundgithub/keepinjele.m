clear
% This script keeps only injecting electrode results from the results
...
load('fulldataRR.mat') % File containing raw results from electrodes
...
for h = 1:819   %length(tconfig)  
    if ismember(h,errorlist)
        sprintf('skipped loopresult02P_%1$d',h)  %change
        continue
    else
        %nothing
    end
    try
        ...
        datacell = [];
        for k = 1:length(elecrtsel)     %loop for all injecting electrode selection
            injele1 = elecrtsel(k,1);
            injele2 = elecrtsel(k,2);

            
            A = (fulldataRR{h}(k,:)); %access results f
            B = [A(injele1) A(injele2)];  %Keep only injecting electrodes
            datacell(k,:) = B;       
            
            
        end
    catch
        sprintf('skipped loopresult%1$d',h) 
    end
    fulldataRROinj{h} = datacell;        
end
save('fulldataRROinj.mat','fulldataRROinj')  %save as fulldataRROinj
