...
for h = 1:length(tconfig)  
    if ismember(h,errorlist)
        sprintf('skipped loopresult_%1$d',h)  %change
        continue
    else
        %nothing
    end
    try
        ...
        for k = 1:length(elecrtsel)    %loop for all injecting electrode selection
            ...
            A = fulldataRR{h}(k,:); %Access the results from data file
            A(injele1) = [];    %remove injecting electrode results
            A(injele2-1) = [];  %-1 as index changed to previous removal of number
            datacell(k,:) = A;
            
        end
    catch
        sprintf('skipped loopresult_%1$d',h)
    end
    fullBdatarinj{h} = datacell;  
end
save('fullBdatarinj.mat','fullBdatarinj')
