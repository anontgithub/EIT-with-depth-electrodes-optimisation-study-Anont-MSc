...
for h = 1:length(tconfig) %loop for all configurations
    if ismember(h,errorlist)
        skipdisp = sprintf('skipped loopresult%1$d',h);
        disp(skipdisp)
        continue
    else
        %nothing
    ...
    try
        ...
        datacell = [];
        for k = 1:length(elecrtsel) %loops all injecting electrode pairs
            
            cfgflname = sprintf('loopresult%1$d_%2$d.txt',h,k);
            datacell(k,:) = readmatrix(cfgflname);           %access textfiles and collates them into one          

        end

        fulldataRR{h} = datacell;         
    catch
...
save('fulldataRR.mat','fulldataRR')  %save collated file as fulldataRR
